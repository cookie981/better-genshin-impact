...
{
    // 当前支持特殊飞行脚本的角色名；默认角色用于未显式写角色名的旧脚本。
    ...
    // 到达特殊飞行动作点前，提前转向/读取动作的距离阈值。
    private const double SpecialFlightPreReadDistance = 15;

    // 快飞/慢飞接近落下点时，允许提前判定到达并取消特殊飞行的缓冲距离。
    ...
    // 特殊飞行状态机：支持特殊飞行起飞/途径/落下，以及飞行消失后的非阻塞接续飞行。
            ...
            for (var i = 0; i < RetryTimes; i++)
            {
                try
                {
                    ...
                    foreach (var waypoint in waypoints) // 一条路径
                    {
                        ...
                        {
                            if (!ReferenceEquals(waypoint, _specialFlightSkipUntilWaypoint))
                            {
                                if (ShouldSkipWaypointBeforeSpecialFlightDropFallback(waypoint))
                                {
                                    continue;
                                }

                                if (await ShouldContinueSpecialFlightSkipToTargetAsync())
                                {
                                    if (!await ShouldRunSpecialFlightViaWaypointAsync(waypoint))
                                    {
                                        continue;
                                    }
                                }
                        ...
                        if (waypoint.Type == WaypointType.Teleport.Code)
                        {
                            ...
                            else if (waypoint.Action == ActionEnum.ForceTp.Code)
                            {
                                AutoFightTask.FightEndFlag = true;
                                AutoFightTask.FightWaypoint = null;
                                await RunForceTeleportCombatScriptAsync(waypoint, nextWaypoint);
                            }
                        }
                        else
                        {
                            ...
                            if (waypoint.Type == WaypointType.Orientation.Code)
                            {
                                ...
                                // 特殊飞行起飞点无需朝向，角色即将起飞，强制旋转无意义
                                var isSpecialFlightStart = HasSpecialFlightAction(waypoint)
                                    && IsSpecialFlightStartAction(GetSpecialFlightAction(waypoint));
    ...
    public async Task MoveTo(WaypointForTrack waypoint,bool isGetOut = true, PathingTask? task = null, Waypoint? nextWaypoint = null,double? nextDistance = null,int retryDis = 4, bool isPoint = true, double? closeDistance = null, string? fastSyncId = null, WaypointForTrack? fastSyncWaypoint = null, bool escapeClimbOnReturn = false, double? returnMoveBudgetSeconds = null)
    {
        ...
        var hasMainAvatarIndex = !string.IsNullOrWhiteSpace(PartyConfig.MainAvatarIndex);
        var skipMainAvatarSwitch = hasMainAvatarIndex && ShouldSkipAvatarSwitchForSpecialFlightState();
        if (skipMainAvatarSwitch)
        {
            Logger.LogInformation("特殊飞行状态中，跳过行走位切换");
        }
        ...
        Task.Run(async () =>
        {
            ...
            if (!skipMainAvatarSwitch && hasMainAvatarIndex && isPoint)
            {
        ...
        if (hasMainAvatarIndex && !skipMainAvatarSwitch && !yellowBlood && _combatScenes?.GetAvatars().Count > 1)
        {
        ...
        while (!ct.IsCancellationRequested)
        {
            ...
            {
                await TryPreReadSpecialFlightCombatScriptAsync(waypoint, nextWaypoint, distance, position);
            }

            await TryPreReadNextSpecialFlightCombatScriptFromTeleportAsync(waypoint, nextWaypoint, distance, nextDistance, position);

            double arriveDistance = !isPoint
                ? (closeDistance.HasValue ? closeDistance.Value : (retryDis > 6 ? 15 : 4))
                : (hurryOnLogo ? 4 : 6);
            if (waypoint is not null
                && GetCurrentSpecialFlightAction(waypoint) == SpecialFlightAction.Drop)
            {
                arriveDistance = Math.Max(arriveDistance, GetSpecialFlightDropArrivalBufferDistance());
            }
            else if (waypoint is not null && ShouldPassSpecialFlightViaEarly(waypoint))
            {
                arriveDistance = Math.Max(arriveDistance, GetSpecialFlightViaArrivalDistance());
            }
        ...
        // 抬起w键
        if (!ShouldKeepMoveForwardForSpecialFlightSlowVia(waypoint))
        {
            Simulation.SendInput.SimulateAction(GIActions.MoveForward, KeyType.KeyUp);
        }
    }
    ...
    private async Task BeforeMoveCloseToTarget(WaypointForTrack waypoint)
    {
        if (waypoint.MoveMode == MoveModeEnum.Fly.Code
            ...
            && !ShouldRunSpecialFlightForStopFlying(waypoint))
        {
